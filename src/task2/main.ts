...
const toStored = (item: AppProduct): StoredProduct => ({
    id: item.id,
    name: item.name,
...
const fromStored = (item: StoredProduct): AppProduct => {
...
const loadInitialProducts = (): AppProduct[] => {
...
const saveProducts = (items: AppProduct[]): void => {
...
let items: AppProduct[] = loadInitialProducts();
...
const field = (labelText: string, control: HTMLElement, className?: string): HTMLDivElement => {
