...
const getAvgRatingText = (productId: number): string => {
...
const getBestDiscountPercent = (categoryId: number, avgRatingText: string): number => {
    ...
    return applicableRules.reduce<number>((best, rule) => {
...
console.log(
    `${pad("product", 16)} | ${pad("category", 16)} | ${pad("supplier", 20)} | ${pad("available", 18)} | ${pad("rating", 10)} | ${pad("specs/description", 55)} | price`
);
